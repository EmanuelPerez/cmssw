...
L1TrackPrimaryVertexProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
 ...
 if( !L1TkTrackHandle.isValid() )
   ...
   float sum2 = -999;
   nPSmin = 0;
   float z2 = MaxPtVertex( L1TkTrackHandle, sum2, nmin, nPSmin, ptmin, imode, theStackedGeometry );
   ...
   nPSmin = 3;
   imode = 1;
   float z3 = MaxPtVertex( L1TkTrackHandle, sum3, nmin, nPSmin, ptmin, imode, theStackedGeometry );
...
/*
  ...
  if (theStackedGeometry == 0) std::cout << " theStackedGeometry = 0 " << std::endl;      // for compil when not used...
*/
